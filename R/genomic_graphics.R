...
circos.genomicDensity = function(
	...
	baseline = 0, 
	border = NA, 
	use_horizon = FALSE, horizon_param = list(),
	...) {
	...
	for(i in seq_along(data)) {
		data[[i]] = validate_data_frame(data[[i]])
	...
	if(length(df) == 1) {
		if(use_horizon) {
			circos.genomicTrackPlotRegion(df[[1]], ylim = c(0, 1), panel.fun = function(region, value, ...) {
				horizon_param$x = (region[, 1] + region[, 2])/2
				horizon_param$y = value[, 1]
				horizon_param$y_max = max(df[[1]][, 4])
				do.call(circos.horizon, horizon_param)
			}, ...)
		} else {
